					...
					if($cel == null) {
							...
							foreach($atak_jednostki as $aj) {
								...
								while($brak && $i > 0) {
									...
									if($cel_jednostki[$i]['jednostka_id'] == $aj['jednostka_id']) {
										$aj['ilosc_wyjscie'] += $cel_jednostki[$i]['ilosc'];
										$brak = false;
									}
									else {
										$i--;
								...
								$sql .= "(".$cel['port_id'].", ".$aj['jednostka_id'].", ".$aj['ilosc_wyjscie'].")";
							}
						...
						//Atakuj¹cy wygrywa i kradnie surowce
								...
								while($row=mysql_fetch_assoc($result)){
									...
									$row['ilosc'] += ($czas - strtotime($row['updated_at'])) / 60 * $row['rate'];
								...
								$atak['status'] = 2;
								$sql_transact = "UPDATE ataki SET status=2 WHERE id=".$atak['id'];
								mysql_query($sql_transact);
